var emiCarousel = {
	'initialize': function() {
		// Wrapper function to get bot list
			function getBotList() {
		...
		// Wrapper function to get bot results
			function getbotResult(id) {
				return new Promise((resolve, reject) => {
					DA.api.EMI.getInsightsByBotId({ botId: id, cb: (err, botResult) => {
						resolve(botResult);
					}});
				});
			}
		...
		// Wrapper function for getting formatted value
		...
		// Wrapper function to set design settings (it's just nicer to use)
			function setDesignSettings(options) {
		...
		// Function for sliding insights
			function slideToInsight(targetId) {
				var currentInsight = d3.select(".insight[position='0']");
				var targetInsight = d3.selectAll('.insight').filter(x => x.id == targetId);
				var currentDot = d3.select(".nav-dot circle[opacity='1']");
				var targetDot = d3.selectAll('.nav-dot circle').filter(x => x.id == targetId);

				var adjust = parseInt(targetInsight.attr('position')) * -1;
				var t = d3.transition();
		...
		// Create the document structure
			var container = d3.select('#__da-app-content').append('div')
				.attr('id', 'container');
			var carousel = container.append('div')
				.attr('id', 'carousel');
			var loader = carousel.append('div')
				.attr('id', 'loader');
			var tooltip = d3.select('#__da-app-content').append('div')
				.attr('id', 'tooltip')
				.style('display', 'none');
			var navDots = container.append('div')
				.attr('id', 'nav-dots');
		...
		// Get and store the selected bot results, and add some text replacement options
				return getbotResult(settings.botSelect);
			}).then(result => {
				// Remove hidden insights
					result.insights = result.insights.filter(insight => {
						var choice = true;
						result.hiddenInsightsCriteria.forEach(criterion => {
				...
				// Validate the insights limit and apply
					if (parseInt(settings.maxInsights) != 'NaN' && parseInt(settings.maxInsights) > 0) {
						result.insights = result.insights.slice(0, parseInt(settings.maxInsights));
					}

				botResult = result;
				metricName = botResult.insightsBotResultConfig.metricName;
				denomMetricName = botResult.insightsBotResultConfig.denominationMetricName;
				numMetricName = botResult.insightsBotResultConfig.numeratorMetricName;
				volumeMetricName = botResult.insightsBotResultConfig.magnitudeMetricName;
		...
		// Get the locale settings, the set measurement format options
				...
				options = options.concat(
					{ 'type': 'select',
						...
						'options': metricFormatOptions(0.0941).slice(1),
						'defaultValue': ',.1%' }
				).concat(
					[metricName, denomMetricName, numMetricName].map((metric, i) => {
						return {
							'type': 'select',
							'id': metric + '_format',
							'displayName': settings[metric] + ' format',
		...
		// Finally, get design settings and create the insights
				...
				// Create the insights
					var insights = carousel.selectAll('div')
							...
							// Create the title
								insights.append('div')
									...
									.on('mouseenter', (event, d) => {
										tooltip
										.style('display', null)
										.text(() => d.map(x => settings[x[0]] + ' ' + x[1]).join(' and '));
									})
									...
									.join('span')
										...
										.join('span')
											...
											.text((d, i) => [settings[d], d][i]);

							// Create the insight text summary
								var insightSummaries = insights.append('div')
												...
												switch(d.analysisType) {
													...
													case 'WIA':
														return d.positive === true
															? 'Caused ' + (botResult.insightsBotResultConfig.expectIncreasingTrend === true ? 'an increase' : 'a decrease') + ' of overall ' + settings[metricName]
															: 'Caused ' + (botResult.insightsBotResultConfig.expectIncreasingTrend === true ? 'a decrease' : 'an increase') + ' of overall ' + settings[metricName];
												}
											});
								...
								// Write the analysis sub type for change-driver insights
									insightSummaries.filter(x => x.analysisType == 'WIA').append('span')
										...
										.on('mouseenter', (event, d) => {
											...
											var compareStart = formatDate(config.comparisonDate.startDate);
											var compareEnd = formatDate(config.comparisonDate.endDate);
											var currentStart = formatDate(config.date.startDate);
											var currentEnd = formatDate(config.date.endDate);

											tooltip
											.style('display', null)
											.text(compareStart + enDash() + compareEnd + ' vs. ' + currentStart + enDash() + currentEnd);
										})
										.on('mousemove', () => tooltip.style('left', event.clientX + 'px').style('top', event.clientY + 'px'))
										.on('mouseleave', () => tooltip.style('display', 'none'))
										.text(d => settings[d.analysisSubType]);

							// Create the insight visualisation elements
								var insightsViz = insights.append('div')
									.attr('class', 'viz-container full-insight');

								// Create the visualisations, starting with performance insights
									var kpiViz = insightsViz.filter(x => x.analysisType == 'KPI').selectAll('div')
									.data(d => { return [
									...
									// Create the judgment text
										...
										kpiVizDesc.append('span')
											...
											.text((d, i) => {
													switch(i) {
														case 0:
															return ' is ' + d3.format(settings.percent_format)(d.summaryValue) + (d.thisValue > d.totalValue ? ' above' : ' below') + ' average.';
														case 1:
															return ' is ' + d3.format(settings.percent_format)(d.summaryValue) + ' of total.';
													}
												});

									// Create the bars visualisations
										var kpiVizBars = insightsViz.selectAll('.kpi-viz:first-child').append('svg')
											.attr('class', 'kpi-viz-bars')
											.datum(d => {
												var max = d3.max([d.thisValue, d.totalValue]);
													return [
														{ 'barColor': settings.foregroundDimension, 'textColor': settings.foregroundText, 'metricName': d.metricName, 'value': d.thisValue, 'max': max },
														{ 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'metricName': d.metricName, 'value': d.totalValue, 'max': max }
													];
											});
										...
										.join('text')
											...
											.text(d => settings[d.metricName + '_format'] !== 0 ? d3.format(settings[d.metricName + '_format'])(d.value) : null)
											.each((d, i, nodes) => {
												if (settings[d.metricName + '_format'] === 0) {
									...
									// Create the pie visualisations
										var kpiVizPie = insightsViz.selectAll('.kpi-viz:nth-child(2)').append('svg')
											.attr('class', 'kpi-viz-pie')
											.attr('viewBox', '0 0 65 65')
											.attr('preserveAspectRatio', 'xMidYMid meet')
											.datum(d => [
												{ 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'metricName': d.metricName, 'value': d.totalValue },
												{ 'barColor': settings.foregroundDimension, 'textColor': settings.foregroundText, 'metricName': d.metricName, 'value': d.thisValue, 'clipId': Math.random().toString(36).substr(2, 9) }
											]);
								...
								// Create the visualisations for change driver insights
									// Create the KPI section
										var wiaResult = insightsViz.filter(x => x.analysisType == 'WIA').append('div')
										...
										// Create the judgment text
											...
											wiaResultOtherSegments.append('span')
												.attr('class', 'metric-name')
												.text(d => settings[metricName]);

											wiaResultOtherSegments.append('span')
												.attr('class', 'metric-desc')
												.text(d => ((d.data.value > d.comparedData.value) ? ' increased': ' decreased') + ' by ' + d3.format(settings.percent_format)((d.data.value - d.comparedData.value) / d.comparedData.value) + '.');
										...
										// Create the result visualisation
											var wiaResultViz = wiaResult.append('svg')
												...
												.datum(d => {
													...
													return [
														{ 'barColor': settings.foregroundDimension, 'textColor': settings.foregroundText, 'value': d.comparedData.value, 'max': max },
														{ 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'value': d.comparedData.totals.value, 'max': max },
														{ 'barColor': settings.foregroundDimension, 'textColor': settings.foregroundText, 'value': d.data.value, 'max': max },
														{ 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'value': d.data.totals.value, 'max': max }
													];
												});
											...
											.join('text')
												...
												.attr('y', d => (1 - d.value / d.max) * 100 * 0.8 + 17 + '%')
												.text(d => settings[metricName + '_format'] !== 0 ? d3.format(settings[metricName + '_format'])(d.value) : null)
												.each((d, i, nodes) => {
													if (settings[metricName + '_format'] === 0) {
									...
									// Create the volume section
										var wiaVolume = insightsViz.filter(x => x.analysisType == 'WIA').append('div')
											.datum(d => { return {
									...
									// Create the judgment text
										var wiaVolumeVizDesc = wiaVolume.append('div')
											.attr('class', 'wia-viz-desc');

										var wiaVolumeNewSegments = wiaVolumeVizDesc.filter(d => d.current === 0 || d.compare === 0);

										wiaVolumeNewSegments.append('span')
											.attr('class', 'metric-name')
											.text(d => settings[d.name]);

										wiaVolumeNewSegments.append('span')
											.attr('class', 'metric-desc')
											.text(d => (d.current === 0 ? ' stopped' : ' started') + ' delivering.');

										var wiaVolumeOtherSegments = wiaVolumeVizDesc.filter(x => x.current !== 0 && x.compare !== 0);
										...
										wiaVolumeOtherSegments.append('span')
											.attr('class', 'metric-desc')
											.text(d => (d.current > d.compare ? ' increased' : ' decreased') + ' by ' + d3.format(settings.percent_format)((d.current - d.compare) / d.compare) + '.');
									...
									// Create the change visualisations
										wiaVolumeViz = wiaVolume.append('svg')
											...
											.datum(d => {
												...
												return [
													{ 'name': d.name, 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'value': d.compareTotal, 'max': max, 'textShift': 0 },
													{ 'name': d.name, 'barColor': settings.foregroundDimension, 'textColor': d.compareTotal / max - d.compare / max < 0.2 ? settings.foregroundTextContrast : settings.foregroundText, 'value': d.compare, 'max': max, 'textShift': d.compareTotal / max - d.compare / max < 0.2 ? 20 : 0 },
													{ 'name': d.name, 'barColor': settings.backgroundDimension, 'textColor': settings.backgroundText, 'value': d.currentTotal, 'max': max, 'textShift': 0 },
													{ 'name': d.name, 'barColor': settings.foregroundDimension, 'textColor': d.currentTotal / max - d.current / max < 0.2 ? settings.foregroundTextContrast : settings.foregroundText, 'value': d.current, 'max': max, 'textShift': d.currentTotal / max - d.current / max < 0.2 ? 20 : 0 }
												];
											});
										...
										.join('text')
											...
											.text(d => settings[d.name + '_format'] !== 0 ? d3.format(settings[d.name + '_format'])(d.value) : null)
											.each((d, i, nodes) => {
												if (settings[d.name + '_format'] === 0) {
					...
					// Function for creating small text-only insights
						function createTextInsights() {
							...
							// Create the text container
								var textContainer = insights.append('div')
								...
								// Create the judgment text
									var textInsightSummaries = textContainer.append('div')
									...
									// Write the KPI change
										var textInsightKPIs = textContainer.append('div')
											.attr('class', 'text-insight-kpi');

										textInsightKPIs.append('span')
											.attr('class', 'metric-name')
											.text(settings[metricName]);

										textInsightKPIs.append('span')
											.attr('class', 'metric-desc')
											.text(d => {
												if (d.analysisType == 'KPI') {
													return ' is ' + d3.format(settings.percent_format)((d.data.value - d.data.totals.value) / d.data.totals.value) + (d.data.value > d.data.totals.value ? ' above' : ' below') + ' average.';
												}
												else if (d.analysisType == 'WIA') {
													return (d.data.value > d.comparedData.value ? ' increased ' : ' decreased ') + 'by ' + d3.format(settings.percent_format)((d.data.value - d.comparedData.value) / d.comparedData.value) + '.';
												}
											});
										...
										.on('mouseenter', (event, d) => {
											tooltip
											.style('display', null)
											.text('Meanwhile, overall ' + settings[metricName] + (d.data.totals.value > d.comparedData.totals.value ? ' increased ' : ' decreased ') + 'by ' + d3.format(settings.percent_format)((d.data.totals.value - d.comparedData.totals.value) / d.comparedData.totals.value) + '.');
										})
										.on('mousemove', () => { tooltip.style('top', event.clientY + 'px').style('left', event.clientX + 'px'); })
										.on('mouseleave', () => { tooltip.style('display', 'none'); });
						}
					...
					// Create initial view based on size
						// Check size, set flex-direction, set visibility
							var size = d3.select('.insight').node().getBoundingClientRect();

							createFullInsights()
							createTextInsights()

							var myStyle = d3.select('head').append('style')
								.html(() => {
									if (size.width < 280 || size.height < 192) {
										return "\
